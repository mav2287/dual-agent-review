...
dar_codex_run() {
  ...
  # The prompt is delivered on STDIN (argv ends with `-`), NOT as an argv element: a
  # large review context passed as a single argument exceeds Linux's MAX_ARG_STRLEN
  # (~128 KB) and fails with E2BIG (macOS has no per-arg cap, so this only bit Linux).
  # stdin has no such limit. printf provides EOF so codex does not hang; stderr MUST go
  # to a file (never /dev/null) or a wedge is invisible.
  dar_codex_argv "$repo" "$schema"
  printf '%s' "$prompt" | "$DAR_CODEX_BIN" "${DAR_CODEX_ARGV[@]}" >"$out" 2>"$err"
  rc=${PIPESTATUS[1]}
  return "$rc"
}

# dar_codex_argv REPO SCHEMA — build the codex argv into the global array
# DAR_CODEX_ARGV. This is the SINGLE place codex flags are assembled, so both the
# real run and the doctor smoke test exercise the identical construction. The argv ends
# with `-` so codex reads the prompt from stdin (see dar_codex_run) — the prompt is
# never an argv element, so it can be arbitrarily large.
#
...
dar_codex_selftest() {
  ( set -euo pipefail
    ...
    dar_codex_argv "/dar/probe/repo" "/dar/probe/schema.json"
    [ "${#DAR_CODEX_ARGV[@]}" -ge 6 ] && [ "${DAR_CODEX_ARGV[$(( ${#DAR_CODEX_ARGV[@]} - 1 ))]}" = "-" ] ) >/dev/null 2>&1
}
