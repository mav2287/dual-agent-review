...
// DAR_EXCLUDE (trusted-repo noise filter) applies to EVERY probe path — the session
// delta already filters it, and manual --diff-base/--files probes must agree with
// the automatic gate about what counts as noise. compileList fails secure: a bad
// pattern records a config error (survey) rather than silently keeping or dropping.
const EXCLUDES = compileList(process.env.DAR_EXCLUDE);
if (EXCLUDES.length) changedFiles = changedFiles.filter((f) => !EXCLUDES.some((re) => re.test(f)));
      ...
      for (const grp of groups || []) {
        for (const h of grp.hooks || []) {
          ...
          for (const raw of parts) {
            // Tokenize: shell-form commands ("bash ./hooks/x.sh --flag") carry the
            // script inside one string — matching the whole string would produce a
            // target like "bash ./hooks/x.sh" that never equals a changed path, and
            // the control script would be misclassified as a contained leaf.
...
for (const file of changedFiles) {
  if (!files.has(file)) {
    // Not in the graph — usually a NEW file, which has no consumers yet. A non-code
    // file with a POSITIVE inert classification (.txt/.md/asset allowlist) is proof
    // of containment even so; new CODE and opaque files still survey (their runtime
    // wiring — auto-loading, globbing, config discovery — can't be seen from here).
    if (!isCode(file) && fileIsInert(file)) { resolved.push(file); continue; }
    unresolved.push({ file, why: "not-in-graph" }); continue;
  }
