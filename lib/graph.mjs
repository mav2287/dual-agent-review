...
// Genuinely dependency-INERT files: human text and binary assets whose change has no
// cross-file blast radius. Deliberately NARROW (finding #6) — a file's presence in the
// repo is NOT proof of containment, so anything that is not clearly inert is treated
// as an opaque control surface and surveyed. Extension alone is not enough: dotfiles
// and named control files (.gitignore, CODEOWNERS, agent-instruction Markdown, …) are
// control surfaces, never inert.
const INERT_EXT = new Set([
  ".txt", ".text", ".rst", ".md", ".mdx", ".markdown",
  ".png", ".jpg", ".jpeg", ".gif", ".svg", ".webp", ".ico", ".bmp",
  ".woff", ".woff2", ".ttf", ".otf", ".eot", ".pdf",
]);
// Files that often look inert (no extension, or .md) but change repository behavior.
const NON_INERT_NAMES = new Set([
  ".gitignore", ".gitattributes", ".gitmodules", ".mailmap", ".npmignore",
  ".dockerignore", ".editorconfig", "codeowners", "claude.md", "agents.md",
]);
export function isInert(file) {
  const b = basename(file);
  const lower = b.toLowerCase();
  if (NON_INERT_NAMES.has(lower)) return false;   // control surface masquerading as inert
  if (b.startsWith(".")) return false;            // dotfiles are configuration, not inert
  if (/^(license|licence|copying|notice|authors|contributors)\b/i.test(b)) return true;
  return INERT_EXT.has(extOf(file));
}
...
export function buildConsumerGraph(repo) {
  ...
  // Per-file {attempted, resolved} — lets the probe judge resolution quality in the
  // NEIGHBOURHOOD of the change, not just a repo-wide average (finding #16).
  const perFile = new Map();
  const bump = (file, key) => {
    let e = perFile.get(file); if (!e) { e = { attempted: 0, resolved: 0 }; perFile.set(file, e); } e[key]++;
  };
  ...
  for (const file of tracked) {
    ...
    for (const spec of extractSpecifiers(content, ext)) {
      ...
      if (internal) { attempted++; bump(file, "attempted"); }
      let target = null;
      ...
      if (internal && target) { resolvedCount++; bump(file, "resolved"); }
      addEdge(target, file);
    }
  }

  const confidence = attempted === 0 ? 1 : resolvedCount / attempted;
  return { consumersOf, files: fileSet, builtAtCommit, confidence, perFile };
}
